...
class ProviderExecutionAttempt(Base):
    ...
    __table_args__ = (UniqueConstraint("company_id", "provider_execution_id", "attempt_number", name="uq_provider_execution_attempts_number"), ForeignKeyConstraint(["company_id", "provider_execution_id"], ["provider_executions.company_id", "provider_executions.id"], name="fk_provider_execution_attempts_execution", ondelete="RESTRICT"), CheckConstraint("attempt_number > 0", name="ck_provider_execution_attempts_number"), CheckConstraint("status IN ('running','succeeded','failed','failed_before_send','outcome_uncertain','cancelled')", name="ck_provider_execution_attempts_status"), Index("ix_provider_execution_attempts_execution", "company_id", "provider_execution_id"))
