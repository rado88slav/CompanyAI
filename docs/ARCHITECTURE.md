...
Repository support is complete, but local development secret provisioning and the running backend have not been cut over. That operational change separately requires approval for atomic local secret provisioning, backend image rebuild, backend container recreation, and health/runtime verification. Migration `0012_credential_keyring_expand` remains the applied database head; migration `0013` does not exist. Production keyring provisioning and a controlled re-encryption workflow remain future security work. No real credential or external provider execution was created. `scripts/setup/create-env.sh --force` replaces the entire `.env` and must not be used for key-only rotation.
...
Production deployments still require dedicated keyring provisioning. Repository runtime support is complete, but local and production operational cutover, contract migration `0013` and a controlled re-encryption workflow remain future work.
