...
The development credential-encryption key was safely rotated while `provider_credentials` contained zero rows. Its cryptographic material was subsequently preserved during the atomic local cutover from standalone `CREDENTIAL_ENCRYPTION_KEY` to the active-ID plus secret-keyring contract. The active development key ID is the non-secret identifier `legacy`; the old standalone environment variable is no longer active. Secret values and hashes remain local and must never appear in Git, documentation, logs or configuration errors. The immutable keyring core exists, and expand migration `0012_credential_keyring_expand` is applied to the real development database at head. The nullable `VARCHAR(64)` `encryption_key_id`, non-null `INTEGER` `encryption_revision` with server default `0`, both check constraints and the ordered `(encryption_key_id, id)` index were verified; `provider_credentials` remains empty. `encryption_key_id` intentionally remains nullable during the expand phase.
...
Migration `0012_credential_keyring_expand` remains the applied real database head. Contract migration `0013_credential_keyring_contract` has been created and validated but not applied. It refuses to run while any `provider_credentials.encryption_key_id` is NULL, performs no backfill or decryption, and makes the column mandatory only after that precondition succeeds. Real development application requires separate explicit approval. Production secret-manager/keyring provisioning, controlled re-encryption tooling for environments with historical credentials, old-key retirement and escrow policy, and backup-retention procedures remain future security and operational work. Dashboard implementation has not started automatically. `scripts/setup/create-env.sh --force` replaces the entire `.env` and must not be used for key-only rotation.
...
The local development runtime uses the active-ID plus keyring contract with active ID `legacy`. Production deployments still require dedicated secret-manager/keyring provisioning. Contract migration `0013_credential_keyring_contract` is validated but awaits separately approved real database application; controlled re-encryption, old-key retirement and escrow, and backup-retention procedures remain future work.
