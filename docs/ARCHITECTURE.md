...
Migration `0012_credential_keyring_expand` remains the applied database head; migration `0013` does not exist. Production secret-manager/keyring provisioning, controlled re-encryption tooling, old-key retirement and escrow policy, and backup-retention procedures remain future security and operational work. `scripts/setup/create-env.sh --force` replaces the entire `.env` and must not be used for key-only rotation.
...
The local development runtime uses the active-ID plus keyring contract with active ID `legacy`. Production deployments still require dedicated secret-manager/keyring provisioning. Contract migration `0013`, controlled re-encryption, old-key retirement and escrow, and backup-retention procedures remain future work.
