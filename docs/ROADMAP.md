...
Current provider foundation status: Provider Connections and the dry-run-only Provider Execution foundation are implemented and runtime-verified. The immutable keyring core exists. Expand migration `0012_credential_keyring_expand` remains the applied real database head, so `encryption_key_id` is still nullable there. Contract migration `0013_credential_keyring_contract` is created and validated but not applied; it refuses NULL key-ID references, performs no backfill or decryption, and makes the column NOT NULL only after the precondition succeeds. Real application requires separate explicit approval. Repository runtime support and the real local keyring cutover are complete with active ID `legacy`; health, readiness and database connectivity are verified, with no real credentials or external provider executions created. Re-encryption tooling, production secret-manager provisioning, old-key retirement/escrow and backup-retention remain open. Dashboard work has not started automatically.
