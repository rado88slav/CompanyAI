...
Current provider foundation status: Provider Connections and the dry-run-only Provider Execution foundation are implemented and runtime-verified. The immutable keyring core exists. Expand migration `0012_credential_keyring_expand` is applied to the real development database at head; its nullable key ID, non-negative revision metadata, constraints and ordered composite index were verified while `provider_credentials` remained empty. Repository runtime support now requires `CREDENTIAL_ENCRYPTION_ACTIVE_KEY_ID` plus secret keyring JSON, validates the complete keyring before FastAPI creation and rejects standalone legacy configuration. New credentials use key-ID-bound version 2 with the configured active key and revision `0`; previous keys are decryption-only. Version-1/NULL-key-ID compatibility requires an explicit `legacy` keyring entry. Local secret provisioning and the running backend have not been cut over. Contract migration `0013`, re-encryption tooling and production keyring provisioning remain deferred. Both execution tables contain zero rows, and no real credentials or external provider executions were created during Stage 4.
