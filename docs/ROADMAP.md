...
Current provider foundation status: Provider Connections and the dry-run-only Provider Execution foundation are implemented and runtime-verified. The immutable keyring core exists. Expand migration `0012_credential_keyring_expand` is applied to the real development database at head; `encryption_key_id` intentionally remains nullable during the expand phase. Repository runtime support validates active-ID plus secret-keyring configuration before FastAPI creation and rejects standalone legacy configuration. Real local development provisioning and the running backend are now cut over with the preserved cryptographic key under active ID `legacy`; the one-key runtime keyring, health, readiness and database connectivity were verified after image rebuild and container recreation. No database rows, real credentials or external provider executions were created. Contract migration `0013`, re-encryption tooling, production secret-manager provisioning, old-key retirement/escrow policy and backup-retention procedures remain deferred.
