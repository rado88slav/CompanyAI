...
## First Run
Wenn kein Administrator existiert, zeigt das Dashboard einen setup-required wizard. Schließen Sie ihn einmal ab; es gibt kein default password.
## Backups
...
Email Operations kann eine Generic SMTP/IMAP Test Message previewen, exact test senders und recipients verwalten, ACTIVE/INACTIVE emergency stop state anzeigen, Approval Manager Authorization anfordern, approval status aktualisieren und entweder Simulation oder einen ausdrücklich bestätigten LIVE TEST ausführen. Emergency stop disabling erfordert die typed phrase `DISABLE EMAIL EMERGENCY STOP`; Simulation bleibt verfügbar, während der Stop active ist. Jede Message oder Sandbox Policy Änderung invalidiert Preview und Approval State. Approvals zeigt exact sender, recipient, subject, body, mode und idempotency reference für Review; Policy kann einen anderen authorized administrator verlangen. Allowlist Audit Records enthalten nur operation metadata, changed state und sender/recipient counts. Simulation entschlüsselt keine Credentials, öffnet kein SMTP und sendet keine Email. LIVE TEST kann nach Approval und final typed confirmation genau eine Nachricht senden.
