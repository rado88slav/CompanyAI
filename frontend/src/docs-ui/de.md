...
Email Operations kann eine Generic SMTP/IMAP Test Message previewen, exact test recipients verwalten, Approval Manager Authorization anfordern und entweder Simulation oder einen ausdrücklich bestätigten LIVE TEST ausführen. Allowlist Audit Records enthalten nur operation metadata, changed state und recipient count. Simulation entschlüsselt keine Credentials, öffnet kein SMTP und sendet keine Email. LIVE TEST kann nach Approval und final typed confirmation genau eine Nachricht senden.
