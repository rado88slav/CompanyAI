...
Email Operations kann eine Generic SMTP/IMAP Test Message previewen, exact test recipients verwalten, Approval Manager Authorization anfordern, approval status aktualisieren und entweder Simulation oder einen ausdrücklich bestätigten LIVE TEST ausführen. Jede Message Änderung invalidiert Preview und Approval State. Approvals zeigt exact sender, recipient, subject, body, mode und idempotency reference für Review; Policy kann einen anderen authorized administrator verlangen. Allowlist Audit Records enthalten nur operation metadata, changed state und recipient count. Simulation entschlüsselt keine Credentials, öffnet kein SMTP und sendet keine Email. LIVE TEST kann nach Approval und final typed confirmation genau eine Nachricht senden.
