...
- Mandatory credential-keyring backend work blocking initial dashboard development is complete. Dashboard Stage 1 provides the tested React/TypeScript/Vite shell, read-only Overview, authenticated company-scoped summary API, read-only Provider Connections module, mock email campaign view, controlled Agent Activity page, graphical administrator login/logout and active company selection. The first Agent Runtime vertical slices invoke only deterministic internal `dashboard.summary.read` and `email.campaigns.list` tools through Tool Registry company enablement and record sanitized audit events. Production secret-manager provisioning, controlled re-encryption, old-key retirement/key escrow, backup-retention and live provider integrations remain open.
- Dashboard authentication and active-company context are implemented for the graphical frontend. The shell now uses existing administrator auth endpoints, bootstraps `/auth/me`, lists only selectable active companies through `/company-context/available-companies`, safely falls back from stale company IDs, preserves the selected company across refresh, remounts protected pages when company context changes and clears expired sessions without rendering tokens or credentials.
- Runtime verification for this slice covered frontend typecheck, frontend production build, 17 frontend tests, 386 backend tests, backend health, readiness, Alembic current and Vite proxy smoke. Browser E2E was not added because Playwright/browser tooling is not present in the repository or shell, and installing browser binaries would require an additional external toolchain download.
- Dashboard homepage product polish is implemented. The Overview route is now a professional operations control center with system health cards, active company summary, provider/email/agent/storage status, quick actions, recent activity and safe notifications. It uses the existing read-only dashboard summary and active-company context only; no backend mutation, provider call or real delivery action was introduced.
- Activity Center product slice is implemented. The dashboard now has a protected `/activity` route with a polished chronological timeline, category and severity filters, grouped event cards, expandable allowlisted safe details and clear loading/error/empty states. The existing company activity API now returns normalized sanitized activity events with bounded pagination and safe filters while preserving administrator authentication, active-company context and company activity read authorization. Overview uses the same normalized activity feed for recent operational events and links to the full Activity Center.
- Activity Center validation covered frontend typecheck, frontend production build, 18 frontend tests, 28 focused backend audit/activity tests, synthetic test-only keyring import validation and no browser tooling availability. Playwright/Chromium browser E2E remains blocked because no browser automation package or browser binary is present locally and installing browser binaries would require an external toolchain download.
- System Status product slice is implemented. The dashboard now has a protected `/system-status` route in the main navigation with read-only cards for Backend, Database, Queue, Storage, Providers, Agent Runtime, Lemlist, Future Telephony and Future AI Providers. It reuses the existing authenticated company-scoped dashboard summary client, marks planned or unavailable runtime areas as limited, and introduces no mutation controls or external actions.
- System Status validation covered frontend typecheck, frontend production build and 19 frontend tests. Backend behavior was unchanged for this slice; health, readiness and Alembic current remain part of runtime smoke verification.
- Documentation Center product slice is implemented. The dashboard now has a protected `/documentation` route in the main navigation with a professional help-center layout, article tree, article reader, table of contents, client-side search, match highlighting, copy link, reading progress, previous/next navigation and responsive behavior. Documentation content is Markdown-backed and lives in one source file per language for Bulgarian, English, German and French. Initial complete articles cover Getting Started, Dashboard, Companies, Providers, Agent, Email Campaigns, Approvals, Activity Center, System Status, Security, FAQ and Release Notes.
- Documentation Center validation covered frontend typecheck, frontend production build, 20 frontend tests and the full backend regression suite. Providers, Agent and Email pages now link directly to relevant documentation. Future user-visible features must update the Documentation Center before the feature is considered complete.
- Navigation polish slice is implemented. The authenticated dashboard shell now displays a compact breadcrumb trail in the top bar, preserving the current company selector and sidebar while making nested routes such as Documentation articles easier to orient. Documentation Center Getting Started and Release Notes were updated as part of the same user-visible change.
- Settings product slice is implemented. The protected `/settings` route now provides Profile, Preferences, Appearance, Notifications, Security, Company defaults and Documentation sections. Preferences are safely stored in local browser storage, documentation language is synchronized to the Documentation Center preference, and security actions avoid insecure password or MFA shortcuts.
- Development activity seed data is implemented through `scripts/dev/seed-activity.sh` and `backend/app/services/development_activity_seed.py`. It is development-only, deterministic, company-scoped, repeatable and idempotent, and creates sample Activity Center events for agent execution, approval requested/granted, provider health, read-only email import, system health and user login without credentials or external actions.
...
- Generic SMTP/IMAP credential recovery is implemented. Provider connection responses expose only safe password status (`missing`/`configured`/`not_required`), cards show Set password or Replace password through the encrypted ProviderCredential APIs, SMTP/IMAP tests and activation remain disabled while the password is missing, and non-secret Edit mailbox settings never returns or redisplays the password.
- Generic SMTP/IMAP Set password 409 root cause is fixed in source: provider credential creation no longer writes `encryption_key_id` into audit details, avoiding audit safe-detail rejection and transaction rollback. The API now returns sanitized credential-specific error codes for duplicate, validation and encryption-unavailable cases.
- Local Edition administrator password recovery is implemented as an interactive existing-account CLI. It runs through the existing backend container, selects by exact administrator email, hides replacement password input, uses the application password hashing and strong local password policy, preserves company/provider data and writes a sanitized platform system audit event. Existing administrator JWTs remain stateless and expire naturally because central session revocation is not yet supported.
- The first approval-gated Generic SMTP/IMAP one-message LIVE TEST path is implemented in source. It adds exact recipient allowlist management, Simulation/LIVE TEST UI mode selection, final typed confirmation, fake-tested strict SMTP transport behavior, digest-bound live execution without persisting full body in ProviderExecution, no automatic retry and terminal `failed_before_send` / `outcome_uncertain` states via migration `0015_live_execution_outcomes` (created but not applied in this validation run).
...
- Controlled live mailbox acceptance readiness is implemented for Generic SMTP/IMAP. Provider Connections now presents an explicit operator checklist for create/select, non-secret settings, encrypted masked password storage, separate no-send SMTP and no-mutation IMAP tests, and activation only after both tests pass. No real credential was entered and no live mailbox connection was attempted during implementation.
- The first product-level AI Agents experience is implemented in preview-only mode. The built-in Email Operations Preview Agent can be created from a safe template, inspected with role/runtime/tools/permissions/approval/health metadata, activated or deactivated, given non-secret company instructions, previewed as a structured prompt and run against deterministic synthetic tasks. Forbidden send attempts are blocked by template policy, audited, and never create provider execution or external action.
- OpenClaw remains future work only. `docs/OPENCLAW_ADAPTER_CONTRACT.md` defines the eventual adapter boundary, but no OpenClaw runtime, container control, direct filesystem access, direct database access or external tool execution was added.
- Validation for this slice covered frontend typecheck, frontend production build, 27 frontend tests, 425 backend tests, OpenAPI and Alembic smoke checks, Docker Compose config checks, backend health/readiness, frontend root/proxy health/readiness and whitespace validation. Secret scanning found only documented/test agent credential prefixes, not real secrets.
- Controlled single-message test preparation is implemented in simulation-only mode. Email Operations can preview one Generic SMTP/IMAP test email, create a dry-run Provider Execution plus Approval Manager single-action request, and execute only a dry-run simulation after approval. The backend rejects inactive or untested mailboxes, non-allowlisted sender/recipient pairs, missing subject prefix, duplicate idempotency keys, quota violations, emergency stop and optional working-hours violations. No SMTP connection, credential decryption, live provider execution or real email send is implemented.
- Email Automation now has a simulation-only worker façade that plans due preview actions and records safe audit without creating Provider Execution records or calling any external provider. Dashboard labels this state as worker simulation only.
- Validation for the single-message simulation slice covered frontend typecheck, frontend production build, 28 frontend tests, 434 backend tests, OpenAPI and Alembic smoke checks, Docker Compose config checks, Local Edition status, Local Edition health/readiness/root smoke, Bash syntax checks, whitespace validation and high-signal secret scanning.
